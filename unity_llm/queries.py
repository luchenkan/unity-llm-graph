...
def resolve_target(conn, target: str) -> Dict:
    ...
    if owner_filter:
        # 字段 / 属性 / 事件:名字重名太常见(`Name` / `Id` / `Count` / `OnClick`),
        # **必须**带 owner 才解析,不做「全项目唯一就猜」——那是只有方法名
        # 才相对安全的策略。裸成员名的正确查法是先 `unity_find` 定位 owner。
        for kind in ("field", "property", "event"):
            frows = conn.execute(
...
def _type_scope(conn, node: Dict):
    ...
    elif node["kind"] in ("method", "field", "property", "event"):
...
def _present_node(node: Dict) -> Dict:
    ...
    keep = {
        ...
        "property": ("kind", "owner", "name", "signature", "file"),
        "event": ("kind", "owner", "name", "signature", "file"),
...
def _stale_field(conn, project_root: str, node: Dict) -> Optional[Dict]:
    ...
    elif node.get("kind") in ("method", "field", "property", "event"):
...
def impact(project_root: str, target: str, depth: int = 3,
    ...
    out: Dict = {"target": target, "resolved": _present_node(node),
                 ...
                 "summary": {}}
    if node["kind"] not in ("asset", "type", "method", "field", "property", "event"):
        conn.close()
    ...
    for o in owners:
        ...
        for c in conn.execute(sql, (o,) + confs):
            if c["external"] and not include_external:
                continue
            if node["kind"] in ("method", "field", "property", "event") and c["name"] not in names:
                continue
    ...
    if node["kind"] in ("property", "event"):
        out["summary"]["note_no_call_edge"] = (
            "目标是属性或事件。属性访问(`x.Prop`)、事件订阅(`x.Evt += h`)在 C# 里"
            "都不是方法调用,不产生指向它的调用边,所以「调用方为 0」是语言层面的"
            "正常结果,**不代表没人使用** —— 不要据此判断可以删除。"
            "这两类入库解决的是 find/context 拿得到名字与签名。")
...
def find_refs(project_root: str, target: str, include_low: bool = False,
    ...
    name_clause = (" AND name=?" if node["kind"] in ("method", "field", "property", "event")
                   else "")
    name_args = ((node["name"],)
                 if node["kind"] in ("method", "field", "property", "event") else ())
    for o in owners:
        for c in conn.execute(
    ...
    if node["kind"] in ("property", "event"):
        out["summary"]["note_no_call_edge"] = (
            "目标是属性或事件。属性访问(`x.Prop`)、事件订阅(`x.Evt += h`)在 C# 里"
            "都不是方法调用,不产生指向它的调用边,所以 code_refs 为空是语言层面的"
            "正常结果,**不代表没人使用** —— 不要据此判断可以删除。"
            "要查谁读写/订阅了它,静态调用图给不出答案。")
...
def stats(project_root: str) -> Dict:
    ...
    out = {
        ...
        "types_by_kind": {r["kind"]: r["c"] for r in conn.execute(
            "SELECT kind, count(*) c FROM types GROUP BY kind ORDER BY c DESC")},
